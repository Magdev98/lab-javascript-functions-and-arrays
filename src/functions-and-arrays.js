...
function averageWordLength(arrayOfString) {
  if (!arrayOfString.length) {
    return null;
  }
  let sum = 0;
  for (i = 0; i < arrayOfString.length; i++) {
    sum += arrayOfString[i].length;
  }
  return sum / arrayOfString.length;
}
...
function doesWordExist(wordArray, searchStr) {
  if (!wordArray.length) {
    return null;
  }
  return wordArray.includes(searchStr);
}
...
function howManyTimes(wordArray, searchStr) {
  if (!wordArray.length) {
    return 0;
  }
  let countWord = 0;
